...
**零第三方依赖，无需任何 API Key** —— clone 下来直接跑。

> **「零依赖」指的是依赖策略，不是"没接 LLM"。** LLM Function Calling 已经实现，
> 只是没有引入官方 SDK —— 而是用标准库 `urllib` 直接调用 Anthropic / OpenAI 的
> HTTP 端点。SDK 只是 HTTP 封装，绕过它不代表绕过模型。
> 后续 RAG 阶段可能引入依赖，届时本节同步更新。
...
### Router 评测：一个满分的评测集，等于没有评测集

Router 评测长期停在 32/32。看着漂亮，实际上说明**这套用例已经不能区分任何东西了**——
...
**难例组的期望值写的是正确行为，不是当前行为。红着的用例是待办事项，改成绿的才是自欺。**
...
建难例组的过程中翻出两个**不报错、也不抛异常，就是答得不对**的 bug。
这类错比崩溃更难被发现：系统照常给出一份像模像样的购票建议，
只是那份建议对应的是另一个城市、另一个日期。
